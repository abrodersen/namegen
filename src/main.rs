...
fn run() -> Result<(), &'static str> {
    ...
    let mut rand = rand::os::OsRng::new()
        .map_err(|_| "failed to initialize rng")?;

    let left = rand.gen_range(0, ADJECTIVES.len());
    let right = rand.gen_range(0, NAMES.len());

    println!("{}_{}", ADJECTIVES[left], NAMES[right]);
...
// These adjectives taken from the Moby project.
static ADJECTIVES: &'static [&'static str] = &[
    "admiring",
    "adoring",
    "affectionate",
    "agitated",
...
// These scientists taken from the Moby project.
